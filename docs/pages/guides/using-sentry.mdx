...
### Tag Configuration
Configure Sentry to tag your scope with information about your update. This will allow you to see which errors are happening on which updates in the Sentry dashboard.

Do this in the global scope, as early as possible in your application's lifecycle.
...
Sentry.configureScope((scope) => {
  scope.setTag("expo-update-group-id", Updates.manifest?.metadata?.updateGroup);
  ...
  scope.setTag(
    ...
    Updates.isEmbeddedLaunch
      ...
      : `https://expo.dev/accounts/${Updates.manifest?.extra?.expoClient?.owner}/projects/${Updates.manifest?.extra?.expoClient?.slug}/updates/${Updates.manifest?.metadata?.updateGroup}`
  );
});
```
...
<Diagram
  source="/static/images/guides/sentry-tags.png"
  alt="Sentry update tags"
  disableSrcSet
/>


### Upload Source Maps
After you run `eas update`, you will need to upload the source maps to Sentry. You will need to install the `jq` binary with package managers like apt-get or brew to run the following commands:

```sh
# Add debug_id field to the source maps in the dist folder
find dist -type f -name '*.map' -exec sh -c 'jq ".debug_id = .debugId" "{}" > "{}.tmp" && mv "{}.tmp" "{}"' \;

# Upload the source maps
SENTRY_PROJECT=[your-project] SENTRY_AUTH_TOKEN=[your-token] node_modules/@sentry/cli/bin/sentry-cli sourcemaps upload --debug-id-reference dist/_expo/static/js/ios/*
SENTRY_PROJECT=[your-project] SENTRY_AUTH_TOKEN=[your-token] node_modules/@sentry/cli/bin/sentry-cli sourcemaps upload --debug-id-reference dist/_expo/static/js/android/*
```
